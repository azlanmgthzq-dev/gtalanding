...
export default function MorphPanel({ onSubmit, onClick }: MorphPanelProps) {
    ...
    return (
        <div className="flex items-center justify-center min-h-[50px]" style={{ width: showForm ? FORM_WIDTH : "auto", height: showForm ? FORM_HEIGHT : 50 }}>
            {/* 
         Adjusted outer container to not force fixed dimensions when closed, 
         allowing it to sit nicer in the ChatHero layout.
      */}
            <motion.div
                ref={wrapperRef}
                data-panel
                className={cn(
                    ...
                    // Removed bottom-8 and max-sm:bottom-5 absolute positioning to fit natural flow
                    "rounded-full" // Initial roundness
                )}
                initial={false}
                animate={{
            ...
            >
                <FormContext.Provider value={ctx}>
                    <DockBar />
                    <InputForm inputRef={textareaRef} onSuccess={handleSuccess} />
...
function DockBar() {
    ...
    return (
        // Added cursor-pointer and onClick here to make the whole bar clickable
        <footer
            className={cn(
        ...
        >
            <div className="flex items-center justify-center gap-2 px-3 max-sm:h-10 max-sm:px-2 w-full">
                ...
                {/* Changed from Button to div/span to prevent nested button click conflicts 
                    and removed hover:text-white classes */}
                <div className="flex h-fit flex-1 justify-end rounded-full px-2 !py-0.5 text-white/90">
...
function InputForm({ inputRef, onSuccess }: { inputRef: React.RefObject<HTMLTextAreaElement | null>; onSuccess: () => void }) {
                    ...
                    >
                        <div className="flex justify-between py-1">
                            ...
                            <button
                                type="submit"
                                ref={btnRef}
                                className="text-white/80 right-4 mt-1 flex -translate-y-[3px] cursor-pointer items-center justify-center gap-1 rounded-[12px] bg-transparent pr-1 text-center select-none text-xs hover:text-white"
                            >
                        ...
                        <textarea
                            ...
                            name="message"
                            className="h-full w-full resize-none scroll-py-2 rounded-md p-4 outline-0 bg-transparent text-white placeholder-white/40"
                            required
...
function KeyHint({ children, className }: { children: string; className?: string }) {
    return (
        <kbd
            className={cn(
                "text-white/70 flex h-6 w-fit items-center justify-center rounded-sm border border-white/20 px-[6px] font-sans text-xs",
                className
            )}
        >
            {children}
